...
processed_se = processed_se %>% eQTLUtils::array_normaliseSE()  
...
plot_title_text <- paste0(study_name, " PCA - quantile normalized, log2 | Sample Size: ", nrow(se %>% SummarizedExperiment::colData()))
...
plot_title_text <- paste0(study_name, " MDS - quantile normalized, log2 | Sample Size: ", nrow(se %>% SummarizedExperiment::colData()))
...
sex_qc_df <- eQTLUtils::array_CalculateSexQCDataFrame(se, assay_name = "exprs", filterByCondition = FALSE, normalise_array = TRUE)
...
Sex_QC_plot <- ggplot2::ggplot(sex_qc_df, ggplot2::aes(x=XIST, y=Y_chrom_mean, label = sample_id)) +
  ...
  ggplot2::labs(x="Expression XIST", y="Mean Expression genes on Y", title = paste0(study_name, " DS | quantile normalised, log2, | Sample Size: ", nrow(sex_qc_df))) +
  ggplot2::theme_bw()
```
